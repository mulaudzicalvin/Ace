...
class ZkMachine(models.Model):
	...
	_logger.info("Logger in device connect = "+str(reply_id))
	buf = zk.createHeader(command, chksum, session_id,reply_id, command_string)
        _logger.info("The buffer value is ="+str(buf))
    ...
    def download_attendance(self):
        ...
        for info in self:
            ...
            port = info.port_no
            zk = zklib.ZKLib(machine_ip, port)
	    _logger.info("In get attandance = "+str(machine_ip)+","+str(port))
            ret = zk.connect()
	    _logger.info("The Device status = "+str(ret))
''' 
	    conn = self.device_connect(zk)
	    _logger.info("In conn = "+str(conn))
            if conn:
                zk.enableDevice()
